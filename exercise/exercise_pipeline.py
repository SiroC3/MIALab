...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(
            image_paths.get(structure.BrainImageTypes.T1w), outputPixelType=sitk.sitkFloat32),  # todo: modify here
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(
            image_paths.get(structure.BrainImageTypes.GroundTruth), outputPixelType=sitk.sitkUInt8)  # todo: modify here
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_gt = sitk.Resample(image1=gt_img, referenceImage=atlas_img, transform=registration.transform,
